...
namespace UngDungQuanLyHoSoSinhVien.GiangVien
{
    public partial class frm_GiangVien_DS_SinhVien : Form
    {
        ...
        public frm_GiangVien_DS_SinhVien(byte VaiTro)
        {
            InitializeComponent();
            this.VaiTro = VaiTro;
            ...
            if (this.VaiTro == 3)
            {
                ...
                TK_SinhVien.HienThi_LuaChon(cmb_LopGiangDay, "SELECT * FROM Lop", "TenLop", "MaLop");
            ...
            else
            {
                this.Enabled = false;
                MessageBox.Show(
                    "Bạn không có quyền truy cập vào trang quản lý tài khoản! Vui lòng đăng nhập bằng tài khoản cán bộ để sử dụng chức năng này.",
                    "Quyền truy cập bị từ chối");
                this.Close();
            }
        }

        private void cmb_LopGiangDay_SelectedIndexChanged(object sender, EventArgs e)
        {
            XuLyTaiKhoanSinhVien TK_SinhVien = new XuLyTaiKhoanSinhVien();
            TK_SinhVien.HienThi_DS_SinhVien_TheoLop(dgv_DSSinhVien, $"{cmb_LopGiangDay.SelectedValue}");
        }
        ...
        {
            int dong = dgv_DSSinhVien.CurrentCell?.RowIndex ?? -1;
        ...
        {
            XuLyTaiKhoanSinhVien TK_SinhVien = new XuLyTaiKhoanSinhVien();
            TK_SinhVien.TimMaSV(tb_TimMSSV.Text);
        }
